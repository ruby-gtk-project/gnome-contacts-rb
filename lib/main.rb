...
class App
  ...
  def primary_menu
    @primary_menu ||= Gio::Menu.new.tap do |menu|
      menu.append_section('List Contacts By:', Gio::Menu.new.tap do |section|
  ...
  def finish_setup(backend)
    ...
    GLib::Idle.add do
      ...
      app.activate if app.registered?
      false
    end
  end

  def setup_actions
    WINDOW_ACTIONS.each { |name, accels| add_action(window, 'win', name, accels) }
    APP_ACTIONS.each { |name, accels| add_action(app, 'app', name, accels) }
  ...
  def add_action(scope, prefix, name, accels)
    Gio::SimpleAction.new(name, nil).tap do |action|
      ...
      app.set_accels_for_action("#{prefix}.#{name}", accels) if accels.any?
    end
  end

  # A stateful action, exactly like upstream's { "sort-on", null, "s", "'surname'" }.
  def add_sort_action
    Gio::SimpleAction.new('sort-on', GLib::VariantType.new('s'),
                          GLib::Variant.new(@settings['sort-on-surname'] ? 'surname' : 'first-name')).tap do |action|
  ...
  def action_delete_contact
    @store.selected_contact.then do |contact|
      run_operation(Operations::DeleteOperation.new(@store, [contact])) if contact
      contact_pane.show_contact(nil) if contact
    end
  end
  ...
  def action_export_marked_contacts
    @store.marked_contacts.then { |marked| export_contacts(marked, 'contacts.vcf') if marked.any? }
  end

  def action_export_selected_contact
    @store.selected_contact.then do |contact|
      export_contacts([contact], "#{filename_for(contact)}.vcf") if contact
    end
  end
  ...
  def import_parsed(hashes)
    run_operation(Operations::ImportOperation.new(@store, hashes)).then do
      contacts_list.update_visible_page
      @operations.last_reversable.then { |op| @store.select_contact(op.imported.first) if op&.imported&.any? }
    end
  end

  def export_contacts(contacts, suggested_name)
    Gtk::FileDialog.new.tap do |dialog|
      dialog.title = contacts.length == 1 ? 'Export Contact' : 'Export Contacts'
  ...
  def show_contact_by_id(id)
    @store.contacts.find { |c| c.id == id }.then { |contact| @store.select_contact(contact) if contact }
  end
  ...
  def apply_startup_query
    ...
    @startup_contact_id.then { |id| show_contact_by_id(id) if id }
  end
  ...
  def register_search_provider
    app.dbus_connection.then do |connection|
      search_provider.register(connection) if connection && @search_provider_enabled
    end
  ...
  def update_contact_menu
    @store.selected_contact.then do |contact|
      contact_menu_button.menu_model = contact_menu(contact) if contact
    end
  end
  ...
  def show_undo_toast(operation)
    toast_overlay.add_toast(Adwaita::Toast.new(operation.description).tap do |toast|
  ...
  def update_action_buttons
    ...
    export_button.action_name = @selection_mode ? 'win.export-marked-contacts' : 'win.export-selected-contact'
    delete_button.action_name = @selection_mode ? 'win.delete-marked-contacts' : 'win.delete-contact'

    @store.selected_contact.then do |contact|
      favorite_button.label = contact&.favorite? ? '_Remove Favourite' : '_Favourite'
      favorite_button.action_name = contact&.favorite? ? 'win.unmark-favorite' : 'win.mark-favorite'
    end
  end

  def update_selection_ui
    @store.marked_contacts.length.then do |count|
      selection_title.title = count.zero? ? 'Select Contacts' : "#{count} Selected"
